...
private:
    void update_caches(mapping::iterator& entry, CacheEntry& data) {
        if(entry != _entries.end()) {
            update_frequency_cache(entry->second, data.frequency);
        }
        data.frequency += 1;
        if(entry != _entries.end()) {
        ...
        // insert
        auto insert_before = _frequencies.upper_bound(data.frequency); // yes, insert before the last entry with higher frequency
        ...
        if(_frequencies.count(data.frequency) == 0) {
            _frequencies.emplace(data.frequency, updated_entry);
        }
    }
