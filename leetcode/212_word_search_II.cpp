...
private:
    ...
    auto _find_words_from(vector<vector<char>> const& board, pair<int, int> location, set<pair<int, int>>& visited, vector<string>& found, string& word, Entry& trie_node) -> void {
        if(trie_node.rest.count(board[location.first][location.second]) == 0) {
            return;
        }
        ...
        for(auto const& offset : offsets) {
            ...
            if(next_location.first < 0 || next_location.first >= board.size() || next_location.second < 0 || next_location.second >= board[0].size()) {
                continue;
            }
            if(visited.count(next_location) == 1) {
                continue;
            }
