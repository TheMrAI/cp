...
enum class RobotType { NONE, ORE, CLAY, OBSIDIAN, GEODE };
...
auto get_maximal_geode_count(Blueprint const& blueprint,
  uint32_t const simulation_minutes,
  SimulationPoint simulate_from = SimulationPoint{ 1, ProductionStatus{ 0, 0, 0, 0, 1, 0, 0, 0 } },
  uint32_t const seen_maximum = 0) -> uint32_t
{
  uint32_t maximum_necessary_ore_production = std::max(
    { blueprint.ore_robot.ore, blueprint.clay_robot.ore, blueprint.obsidian_robot.ore, blueprint.geode_robot.ore });
  uint32_t maximum_necessary_clay_production = std::max(
    { blueprint.ore_robot.clay, blueprint.clay_robot.clay, blueprint.obsidian_robot.clay, blueprint.geode_robot.clay });
  uint32_t maximum_necessary_obsidian_production = std::max({ blueprint.ore_robot.obsidian,
  ...
  while (!options.empty()) {
    ...
    auto absolute_maximal_output =
      to_evaluate.status.geode_total
      + sum_with_arithmetic_progression(to_evaluate.status.geode_production, remaining_minutes);
    ...
    if (to_evaluate.minute > simulation_minutes || absolute_maximal_output < maximal_geode_count) { continue; }
    ...
    // if (to_evaluate.status.ore_production >= blueprint.geode_robot.ore && to_evaluate.status.clay_production >=
    // blueprint.geode_robot.clay && to_evaluate.status.obsidian_production >= blueprint.geode_robot.obsidian) {
    ...
    if (can_build_robot(blueprint.ore_robot, to_evaluate.status)
        && to_evaluate.status.ore_production < maximum_necessary_ore_production) {
      auto after_update = update_status(to_evaluate, blueprint.ore_robot);
      maximal_geode_count = std::max(maximal_geode_count, after_update.status.geode_total);
      options.push(after_update);
    }
    if (can_build_robot(blueprint.clay_robot, to_evaluate.status)
        && to_evaluate.status.clay_production < maximum_necessary_clay_production) {
      auto after_update = update_status(to_evaluate, blueprint.clay_robot);
      maximal_geode_count = std::max(maximal_geode_count, after_update.status.geode_total);
      options.push(after_update);
    }
    if (can_build_robot(blueprint.obsidian_robot, to_evaluate.status)
        && to_evaluate.status.obsidian_production < maximum_necessary_obsidian_production) {
      auto after_update = update_status(to_evaluate, blueprint.obsidian_robot);
      // maximal_geode_count = std::max(maximal_geode_count, after_update.status.geode_total);
      // options.push(after_update);
      maximal_geode_count = std::max({ maximal_geode_count,
        after_update.status.geode_total,
        get_maximal_geode_count(blueprint, simulation_minutes, after_update, maximal_geode_count) });
    }
...
auto main() -> int
{
  ...
  while (std::getline(std::cin, line)) {
    ...
    auto obsidian_robot = Robot{
      RobotType::OBSIDIAN, static_cast<uint32_t>(std::stoi(match[4])), static_cast<uint32_t>(std::stoi(match[5])), 0
    };
    auto geode_robot = Robot{
      RobotType::GEODE, static_cast<uint32_t>(std::stoi(match[6])), 0, static_cast<uint32_t>(std::stoi(match[7]))
    };

    auto blueprint = Blueprint{ blueprint_id, none_robot, ore_robot, clay_robot, obsidian_robot, geode_robot };
