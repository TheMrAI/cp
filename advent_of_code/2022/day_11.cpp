...
auto main() -> int {
        ...
        for(auto i = 0; i < 6; ++i) {
            ...
            switch(i) {
                ...
                case 1: {
                    ...
                    for(auto entry = items_begin; entry != items_end; ++entry) {
                        monkey.items.push(stoi(entry->str()));
                    } 
                    break;
                }
                case 2: {
                    ...
                    if (assigned == 2) {
                        monkey.modify_by.emplace(modify_by);
                    }
                    break;
                }
                case 3: {
    ...
    for(auto i=0; i < 10'000; ++i) {
        for(auto mi=0; mi < monkeys.size(); ++mi) {
            while(!monkeys[mi].items.empty()) {
                ...
                if(monkeys[mi].modify_by.has_value()) {
                    modify_by = monkeys[mi].modify_by.value();
                }
    ...
    cout << "Monkey business: " << inspect_counter[inspect_counter.size()-1] * inspect_counter[inspect_counter.size()-2] << endl;
