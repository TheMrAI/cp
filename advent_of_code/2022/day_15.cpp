...
auto locate_beacon(vector<LineSegments> const& segments,
  int64_t l_x,
  int64_t h_x,
  int64_t l_y,
  int64_t h_y,
  pair<int64_t, int64_t>& beacon_location) -> bool
{
  if (l_x == h_x || l_y == h_y) { return false; }

  auto all_corners_covered_by_one_segment = false;
  auto corners =
    vector<pair<int64_t, int64_t>>{ { l_x, l_y }, { l_x, h_y - 1 }, { h_x - 1, l_y }, { h_x - 1, h_y - 1 } };
  for (auto const& segment : segments) {
    all_corners_covered_by_one_segment = std::all_of(corners.cbegin(), corners.cend(), [segment](auto const& corner) {
      return within_beacon_range(segment, corner.first, corner.second);
    });
    if (all_corners_covered_by_one_segment) { break; }
  }

  if (all_corners_covered_by_one_segment) { return false; }
...
auto main() -> int
{
  ...
  while (getline(cin, line)) {
    ...
    for (auto i = numbers_begin; i != numbers_end; ++i) { numbers.push_back(atoll(i->str().c_str())); }
    ...
    line_segments.emplace_back(sensor_x - manhattan_distance,
      sensor_x + manhattan_distance,
      sensor_y - manhattan_distance,
      sensor_y + manhattan_distance,
      sensor_x,
      sensor_y,
      beacon_x,
  ...
  ranges::sort(
    line_segments.begin(), line_segments.end(), [](auto const& lhs, auto const& rhs) { return lhs.y_s < rhs.y_s; });
  auto candidates_at_line_y = vector<LineSegments>{};
  std::copy_if(line_segments.cbegin(),
    line_segments.cend(),
    std::back_inserter(candidates_at_line_y),
    [target_y](auto const& segment) { return segment.y_s <= target_y && target_y <= segment.y_u; });
  ranges::sort(candidates_at_line_y.begin(), candidates_at_line_y.end(), [](auto const& lhs, auto const& rhs) {
  ...
  for (auto const& segment : candidates_at_line_y) {
    ...
    if (segment_until <= covered_until) { continue; }
