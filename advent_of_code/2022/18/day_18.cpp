...
auto main() -> int
{
  ...
  constexpr auto offsets = std::array<Triplet, 6>{ Triplet{ 0, 0, 1 },
    Triplet{ 0, 0, -1 },
    Triplet{ 0, 1, 0 },
    Triplet{ 0, -1, 0 },
    Triplet{ 1, 0, 0 },
  ...
  while (getline(cin, line)) {
    ...
    if (droplet_voxels.contains(key)) { continue; }
    ...
    for (auto const& offset : offsets) {
      ...
      if (droplet_voxels.contains(key)) { side_count -= 2; }
    }
  }
  // Part 1
  ...
  while (!to_check.empty()) {
    ...
    if (bounding_voxels.contains(checking_key)) { continue; }
    ...
    for (auto const& offset : offsets) {
      ...
      if (bounding_voxels.contains(candidate_key)) { bounding_object_side_count -= 2; }
      // skip all invalid voxels from side counting
      if (droplet_voxels.contains(candidate_key) || candidate_x < min_x || candidate_x > max_x || candidate_y < min_y
          || candidate_y > max_y || candidate_z < min_z || candidate_z > max_z) {
        continue;
      }
  ...
  auto droplet_outside_perimeter =
    bounding_object_side_count - (2 * delta_x * delta_y) - (2 * delta_z * delta_y) - (2 * delta_x * delta_z);
