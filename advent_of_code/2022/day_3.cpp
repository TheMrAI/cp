...
{
  if (std::islower(input)) { return 1 + input - 'a'; }
  return 27 + input - 'A';
}
...
auto find_duplicate(std::string input) -> char
{
  ...
  while (index < input.size()) {
    ...
    if (seen[seen_index]) { return input[index]; }
    ++index;
  }
  return '!';
}
  ...
  while (std::getline(std::cin, input)) {
    ...
    if (triplets.size() == 3) {
      ...
      std::set_intersection(
        first.cbegin(), first.cend(), second.cbegin(), second.cend(), std::back_inserter(intersection));
      auto final_group = std::vector<char>{};
      std::set_intersection(
        intersection.cbegin(), intersection.cend(), third.cbegin(), third.cend(), std::back_inserter(final_group));
      ...
      if (final_group.size() != 1) {
        ...
        for (auto const c : final_group) { std::cerr << c << std::endl; }
      }
