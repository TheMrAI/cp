...
{
  auto tokens_begin = std::sregex_iterator(input.begin(), input.end(), tokens_regex);
...
auto smaller(Item const& left, Item const& right) -> int
{
  ...
  if (!left.is_number && !right.is_number) {
    ...
    if (left.items.size() == right.items.size()) { return 0; }
    return 1;
  }
  if (left.is_number && !right.is_number) {
...
auto main() -> int
{
  ...
  sort(
    packets.begin(), packets.end(), [](auto const& lhs, auto const& rhs) -> bool { return smaller(lhs, rhs) == -1; });
