...
auto calculate_total_sum(vector<vector<FileOrDir>> const& filesystem, int in_dir, uint64_t& total_dir_sum) -> uint64_t
{
  ...
  if (local_dir_sum <= 100'000) { total_dir_sum += local_dir_sum; }

  return local_dir_sum;
}

// Part 2
auto find_smallest_dir_for_delete(vector<vector<FileOrDir>> const& filesystem,
  int in_dir,
  uint64_t space_needed,
  uint64_t& smallest_sufficient_dir_size) -> uint64_t
{
  auto local_dir_sum = uint64_t{ 0 };
  for (auto const& entry : filesystem[in_dir]) {
    if (entry.dir) {
      local_dir_sum +=
        find_smallest_dir_for_delete(filesystem, entry.size_or_index, space_needed, smallest_sufficient_dir_size);
    } else {
      local_dir_sum += entry.size_or_index;
    }
  }
    ...
    if (regex_match(line, match_result, cd_regex)) {
      ...
      for (auto entry = sregex_iterator(path.begin(), path.end(), path_regex); entry != sregex_iterator{}; ++entry) {
        ...
        if (move_to == string{ ".." }) {
          ...
          auto dir = find_if(filesystem[location.top()].begin(),
            filesystem[location.top()].end(),
            [&move_to](auto val) { return val.dir == true && val.name == move_to; });
      ...
      filesystem[location.top()].push_back(
        FileOrDir{ true, static_cast<int>(filesystem.size()) - 1, match_result[1].str() });
