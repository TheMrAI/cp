...
auto main() -> int {
    ...
    while(getline(cin, line)) {
        geography.emplace_back(line.begin(), line.end());
    }
    ...
    for(auto i = 0; i < geography.size(); ++i) {
        for(auto j=0; j < geography[0].size(); ++j) {
            ...
            if (j < geography[0].size() - 1) {
                ...
                if (right_node_height - node_height <= 1) {
                    edge_list[node_id].push_back(right_node_id);
                }
                if (node_height - right_node_height <= 1) {
                    edge_list[right_node_id].push_back(node_id);
                }
            }
            // down
            if (i < geography.size() -1) {
                ...
                if (down_node_height - node_height <= 1) {
                    edge_list[node_id].push_back(down_node_id);
                }
                if (node_height - down_node_height <= 1) {
                    edge_list[down_node_id].push_back(node_id);
                }
            }
        }      
    }
    ...
    for (auto i =0; i<geography.size(); ++i) {
        for(auto j=0; j<geography[0].size(); ++j) {
            if(geography[i][j] == 'a') {
                ...
                if (path_length == numeric_limits<int>::min()) {
                    continue;
                }
