...
auto move_element(std::list<int64_t>& ordering,
  std::unordered_map<size_t, std::list<int64_t>::iterator>& index_map,
  size_t index,
  bool move_right) -> void
{
  ...
  if (moves_to_make == 0) { return; }

  auto walker = ordering.erase(element);
  if (move_right && walker == ordering.end()) { walker = ordering.begin(); }
  if (!move_right && walker == ordering.begin()) { walker = ordering.end(); }

  auto offset = 1;
  if (!move_right) { offset = -1; }
...
auto solve(std::vector<int64_t> numbers, int64_t decryption_key, int64_t mixing_times) -> void
{
  ...
  for (auto mixing_count = 0; mixing_count < mixing_times; ++mixing_count) {
    for (auto i = size_t{ 0 }; i < numbers.size(); ++i) {
      ...
      if (numbers[i] < 0) { move_right = false; }
      move_element(ordering, index_map, i, move_right);
    }
  }

  numbers.clear();
  for (auto entry : ordering) { numbers.push_back(entry); }
  // dump_to(std::cout, numbers.begin(), numbers.end());
  // std::cout << std::endl;
