...
auto main() -> int
{
  ...
  while (getline(cin, row)) {
    ...
    for (auto i = 0; i < row.size(); ++i) { tree_heights[i] = row[i] - '0'; }
  ...
  for (auto i = 0; i < forest.size(); ++i) {
    for (auto j = 1; j < forest[0].size(); ++j) { left_heights[i][j] = max(left_heights[i][j - 1], forest[i][j - 1]); }
  }
  ...
  for (auto j = 0; j < forest[0].size(); ++j) {
    for (auto i = 1; i < forest.size(); ++i) { up_heights[i][j] = max(up_heights[i - 1][j], forest[i - 1][j]); }
  }
  ...
  for (auto i = 0; i < forest.size(); ++i) {
    for (auto j = 0; j < forest[0].size(); ++j) {
      // cout << forest[i][j];
      if (left_heights[i][j] < forest[i][j] || right_heights[i][j] < forest[i][j] || up_heights[i][j] < forest[i][j]
