...
auto dfs_distance(vector<vector<int>>const& edge_list, int node, int from, vector<i6>& distances, int& farthest_node) -> void {
    for(auto const& neighbour : edge_list[node]) {
        if(neighbour == from) {
            continue;
        }
        distances[neighbour] = distances[node] + 1;
        if(distances[farthest_node] < distances[neighbour]) {
...
auto main() -> int {
    ...
    while(t--) {
        ...
        for(auto i = 0; i < n; ++i) {
            max_profit = max(max_profit, (max(distances_from_node_a[i], distances_from_node_b[i]) * k) - (distances_from_root[i]*c));
        }
