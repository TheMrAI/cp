    ...
    while(t--) {
        ...
        while(start < end) {
            if(data[start] == data[end]) {
                invalid_chars[data[start]].push_back(start);
            }
            ++start;
            --end;
        }
        auto swap_count = 0;
        auto by_length = [](auto const& lhs, auto const& rhs) {
            return lhs.size() < rhs.size();
        };
        auto char_indexes = priority_queue<vector<int>, vector<vector<int>>, decltype(by_length)>{by_length};
        for(auto& [_, v] : invalid_chars) {
            char_indexes.push(move(v));
        }
        while(!char_indexes.empty() && char_indexes.size() >= 2) {
            ...
            if(!most_common.empty()) {
                char_indexes.push(most_common);
            }
            if(!second_most_common.empty()) {
                char_indexes.push(second_most_common);
            }
        }
