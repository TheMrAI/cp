...
// too much memory!!!
// auto _find_sequence(vector<vector<int>> const & days, vector<int>& sequence, int day, unordered_set<int> const &
// seen) -> bool {
//     if(day < 0) {
//         return true;
//     }
...
auto main() -> int
{
  ...
  while (t--) {
    ...
    if (ok) {
      for (auto v = winner_sequence.begin(); v != winner_sequence.end(); ++v) { cout << *v << " "; }
      cout << endl;
    } else {
      cout << -1 << endl;
    }
