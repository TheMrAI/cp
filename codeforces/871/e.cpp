...
auto dfs_largest(vector<vector<int>>& matrix, int i, int j, int& depth, int& largest_lake) -> void
{
  ...
  if (matrix[i][j] == -1 || matrix[i][j] == 0) { return; }
  depth += matrix[i][j];
  largest_lake = max(largest_lake, depth);
  matrix[i][j] = -1;
  constexpr auto dirs =
    array<pair<int, int>, 4>{ make_pair(1, 0), make_pair(-1, 0), make_pair(0, 1), make_pair(0, -1) };
  for (auto const& dir : dirs) { dfs_largest(matrix, i + dir.first, j + dir.second, depth, largest_lake); }
}
...
auto main() -> int
{
  ...
  while (t--) {
    ...
    for (auto i = 0; i < n; ++i) {
      for (auto j = 0; j < m; ++j) { cin >> matrix[i][j]; }
    }
    auto largest_lake = 0;
    for (auto i = 0; i < n; ++i) {
      for (auto j = 0; j < m; ++j) {
