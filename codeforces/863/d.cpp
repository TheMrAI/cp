...
auto valid_fn(array<int64_t, 46> const& fibs, int64_t n, int64_t x, int64_t y) -> bool
{
  ...
  if (y > height) { return valid_fn(fibs, n - 1, y - height, x); }
  if (width - y >= height) { return valid_fn(fibs, n - 1, y, x); }
  return false;
}
...
auto main() -> int
{
  ...
  fib_array[0] = 1;
  fib_array[1] = 1;
  for (auto i = 2; i < 46; ++i) { fib_array[i] = fib_array[i - 1] + fib_array[i - 2]; }
