...
auto main() -> int
{
  ...
  for (auto i = 0; i < 26; ++i) {
    ...
    copy_if(odd_length_words.begin(), odd_length_words.end(), back_inserter(filtered_odd), [i](auto const& entry) {
      return (entry.first & (1 << i)) == 0;
    });
    auto filtered_even = vector<pair<bitset<26>, uint32_t>>{};
    copy_if(even_length_words.begin(), even_length_words.end(), back_inserter(filtered_even), [i](auto const& entry) {
      return (entry.first & (1 << i)) == 0;
    });
    ...
    for (auto const& odd_word : filtered_odd) {
      ...
      auto match_start =
        lower_bound(filtered_even.cbegin(), filtered_even.cend(), target, [](auto const& entry, auto const& target) {
          return entry.second < target;
        });
      auto match_end =
        upper_bound(filtered_even.cbegin(), filtered_even.cend(), target, [](auto const& target, auto const& entry) {
