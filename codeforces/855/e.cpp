...
auto make_set(vector<i6>& parent, i6 value) -> void { parent[value] = value; }
...
auto union_sets(vector<i6>& parents, vector<i6>& ranks, i6 lhs, i6 rhs) -> void
{
  ...
  if (parent_lhs != parent_rhs) {
    if (ranks[parent_lhs] < ranks[parent_rhs]) { swap(parent_lhs, parent_rhs); }
    parents[parent_rhs] = parent_lhs;
    if (ranks[parent_lhs] == ranks[parent_rhs]) { ++ranks[parent_lhs]; }
  }
}
...
auto main() -> int
{
  ...
  while (t--) {
    ...
    for (auto c : spell) { ++spell_chars[c - 'a']; }
    auto to_spell_chars = vector<int>(26, 0);
    for (auto c : to_spell) { ++to_spell_chars[c - 'a']; }
    ...
    for (auto i = 0; i < spell.size(); ++i) { make_set(parents, i); }
    for (auto i = 0; i < spell.size(); ++i) {
      if (0 <= i - k) { union_sets(parents, ranks, i, i - k); }
      if (0 <= i - k - 1) { union_sets(parents, ranks, i, i - k - 1); }
      if (i + k <= spell.size() - 1) { union_sets(parents, ranks, i, i + k); }
      if (i + k + 1 <= spell.size() - 1) { union_sets(parents, ranks, i, i + k + 1); }
    }
    ...
    for (auto i = 0; i < spell.size(); ++i) {
      ...
      if (counter.size() == 0) { counter.assign(26, 0); }
      ++counter[spell[i] - 'a'];
    }
