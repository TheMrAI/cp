...
auto main() -> int
{
  ...
  while (t--) {
    ...
    if (distinct_numbers.size() <= 1) {
      ...
      if (distinct_zero_indexes.size() != 0 && distinct_numbers.size() == 0) { max_power += 1; }

      auto result = int64_t(1);
      auto mod = static_cast<int64_t>(1e9 + 7);
      for (auto i = int64_t{ 0 }; i < max_power; ++i) { result = ((result % mod) * (m % mod)); }
      // cout << "Indexes: " << distinct_zero_indexes.size() << " Numbers: " << distinct_numbers.size() << " POW: " <<
      // max_power << endl;
